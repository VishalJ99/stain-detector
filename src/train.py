...
from config import parse_args, load_config, update_config_with_args

def train_epoch(model, train_loader, criterion, optimizer, device, config, epoch, logger):
    """
...
def validate(model, val_loader, criterion, device, config, epoch, logger):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(val_loader):
            ...
            if batch_idx == 0 and epoch % 5 == 0:
                ...
                log_batch_images(inputs, targets, predicted, val_loader.dataset.classes, epoch, prefix="val")
    ...
    logger.info(f"Validation - Epoch {epoch+1}/{config.training.num_epochs}, "
...
def train(config, args):
    ...
    run_name = config.run_name if config.run_name else f"stain_detector_{timestamp}"
    ...
    try:
        ...
        logger.info(f"Initialized Weights & Biases logging to project: {config.logging.wandb_project}")
    except Exception as e:
        logger.warning(f"Failed to initialize Weights & Biases: {str(e)}")
        logger.warning("Continuing without W&B logging. Set WANDB_API_KEY in .env file to enable.")
    ...
    for epoch in range(config.training.num_epochs):
        # Train for one epoch
        train_loss = train_epoch(model, train_loader, criterion, optimizer, device, config, epoch, logger)
        
        # Validate
        val_loss, val_accuracy = validate(model, val_loader, criterion, device, config, epoch, logger)
