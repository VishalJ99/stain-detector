...
def extract_patches_from_wsi(wsi_path, patch_size=256, overlap=0.25, level=0, tissue_threshold=0.05, 
                            ...
                            exclusion_conditions=None, exclusion_mode='any', extraction_mode='random',
                            save_patches=False, output_dir=None, label=None):
    """
    ...
    if exclusion_mode not in ['any', 'all']:
        print(f"Warning: Invalid exclusion_mode '{exclusion_mode}', defaulting to 'any'")
        exclusion_mode = 'any'
        
    # Validate extraction_mode
    if extraction_mode not in ['random', 'contiguous']:
        print(f"Warning: Invalid extraction_mode '{extraction_mode}', defaulting to 'random'")
        extraction_mode = 'random'
    
    # Setup for saving patches
    metadata = {}
    if save_patches:
        assert output_dir is not None, "output_dir must be provided when save_patches=True"
    ...
    if extraction_mode == 'random':
        ...
        while count < num_patches and attempts < max_attempts:
            ...
            if exclusion_mode == 'any' and any(satisfied_conditions):
                should_exclude = True
            elif exclusion_mode == 'all' and all(satisfied_conditions) and satisfied_conditions:
        ...
        print(f"Creating {num_patches_x}x{num_patches_y} grid of patches with step size {step_size}")
        
        # Add this in the contiguous mode section, right after calculating num_patches_x and num_patches_y
        if create_debug_images:
            print(f"Full grid would be {num_patches_x}x{num_patches_y} = {num_patches_x*num_patches_y} patches")
        ...
        for y_idx in range(num_patches_y):
            for x_idx in range(num_patches_x):
                ...
                patch_mask = dilated_tissue_mask[thumb_y:thumb_y+thumb_patch_size, thumb_x:thumb_x+thumb_patch_size]
        ...
        for x_idx, y_idx, thumb_x, thumb_y in roi_positions:
            ...
            if should_infer:
                # Store patch with coordinates
                patches.append((patch_np, full_x, full_y))
                if count % 100 == 0:  # Print every 100 patches to avoid flooding console
                    print(f"Patch {count} at position x={full_x}, y={full_y} is a tissue patch")
...
if isinstance(result, tuple):
    ...
    if patches:
        ...
        print(f"First patch is at coordinates x={x}, y={y} with shape {first_patch.shape}")
